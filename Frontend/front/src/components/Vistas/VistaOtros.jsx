...
const VistaOtros = () =>{
  ...
  const [otros, setOtros] = useState([]);//Lista de servicios
  ...
  const guardarTransporte = async () => {
    try {
      const response = await axios.post("/operaciones/confirmar/{operacionId}", otros);
    ...
    return(
      ...
      <Container maxWidth="sm">
        <Paper elevation={3} style={{ padding: "20px", marginTop: "20px" }}>
          ...
          <Typography variant="h5" gutterBottom>
            {editIndex !== null ? "Editar Gasto" : "Registrar Otros Gastos"}
          </Typography>
      ...
      <List>
        {otros.map((otro, index) => (
          ...
          >
            <ListItemText
              primary={`${otro.descripcion}`}
              secondary={`Fecha: ${otro.fecha} | Monto: $${otro.monto.toFixed(2)}`}
            />
          </ListItem>
